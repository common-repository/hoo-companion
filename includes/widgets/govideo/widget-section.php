...
// Creating the widget 
class HoocSectionPosts_Widget extends WP_Widget {
...
function __construct() {
	...
	__('HC: Section Posts', 'hoo-companion'), 
	 
	array( 'description' => __( 'Section Posts.', 'hoo-companion' ), ) 
	);
}
...
public function widget( $args, $instance ) {
	...
	$defaults = array( 'section_title' => '', 'category' => '0', 'posts_count' => '8','taxonomy' =>'category', 'order_by' => 'date', 'style' => '0' );
	...
	if( !$image )
		$image = HooCompanion::get_featured_image();
			...
			$large_post = '<article id="post-'.$post_id.'" class="'.join( ' ', get_post_class( 'large-post clearfix', $post_id ) ).'">
									<div class="wrap-vid">
									  <div class="zoom-container">
										<div class="zoom-caption">
										<span>'.esc_attr($first_tag).'</span>
										<a href="'.esc_url($link).'"> '.HooCompanion::get_zoom_icon().' </a>
										  <p class="overlay-title">'.esc_attr($title).'</p>
										</div>
										'.$image.'
									</div>
									...
									<p class="more">'.$excerpt.'</p>
									<a href="'.esc_url($link).'" class="btn btn-1">'.__( 'Read More', 'hoo-companion' ).'</a>
									
									'.(( $instance['style'] == 2 )?'<div class="line"></div>':'').'
		...
		case "2":
			if( $i%2 == 0 ){
				if( $i==0 )
					$left_column .= $large_post;
				else
					$left_column .= $small_post;	
					
			}else{
				if( $i==1 )
					$right_column .= $large_post;
				else
					$right_column .= $small_post;
			}
		break;
		case "1":
			if( $i == 0 )
		...
		break;
		default:
		$html .= '<div class="col-md-4">'.$small_post_2.'</div>';
		break;
	}
    ...
    <?php if( $section_title != '' ):?>
        <div class="box-header">
            <h2><i class="fa fa-globe"></i> <?php echo esc_attr( $section_title ); ?></h2>
        </div>
        <?php endif;?>
...
public function form( $instance ) {
	...
	?>
    <p>
        <label for="<?php echo $this->get_field_id( 'section_title'  ); ?>"><?php _e('Section Title', 'hoo-companion'); ?>:</label>
         <br />
		<input id="<?php echo $this->get_field_id( 'section_title'  ); ?>" name="<?php echo $this->get_field_name( 'section_title'  ); ?>" value="<?php echo esc_attr( $instance['section_title']); ?>" class="widefat" /> 
        </p>
    ...
    <br />
     <select id="<?php echo $this->get_field_id( 'style' ); ?>" name="<?php echo $this->get_field_name( 'style'  ); ?>">
       ...
       <option   value="0" <?php selected( $instance['style'], '0' ,true ); ?>><?php _e( 'Style 1 (No Featured Post)', 'hoo-companion' ); ?></option>
       <option   value="1" <?php selected( $instance['style'], '1' ,true ); ?>><?php _e( 'Style 2 (One Featured Post)', 'hoo-companion' ); ?></option>
       
       <option   value="2" <?php selected( $instance['style'], '2' ,true ); ?>><?php _e( 'Style 3 (Two Featured Posts)', 'hoo-companion' ); ?></option>
...
public function update( $new_instance, $old_instance ) {
	...
	$instance['section_title'] = ( ! empty( $new_instance['section_title'] ) ) ? esc_attr( $new_instance['section_title'] ) : '';
