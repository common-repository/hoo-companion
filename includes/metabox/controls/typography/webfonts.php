 ...
 * @since 1.0.0
 */
function hoo_standard_fonts() {
	return apply_filters( 'hoo_standard_fonts_array', array(
 ...
 * @since 1.0.0
 */
function hoo_enqueue_google_font( $font ) {

	// Return if disabled
	if ( true == get_theme_mod( 'hoo_disable_google_font', false ) ) {
		return;
	}

	// Get list of all Google Fonts
	$google_fonts = hoo_google_fonts_array();
	...
	// Subset
	$get_subsets 	= get_theme_mod( 'hoo_google_font_subsets', array( 'latin' ) );
	$subsets 		= '';
	...
	$weights = apply_filters( 'hoo_google_font_enqueue_weights', $weights, $font );
	$italics = apply_filters( 'hoo_google_font_enqueue_italics', true );
	...
	// Enqueue style
	wp_enqueue_style( 'hoocompanion-google-font-'. $handle, $url, false, false, 'all' );
