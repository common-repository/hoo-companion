( function() {
	...
	butterbean.views.register_control( 'select', {
        ...
        ready : function() {
            ...
            jQuery( this.$el ).find( '.butterbean-select' ).owpSelect2( {
                ...
                dropdownCssClass: 'hoocompanion-select2',
                width: '100%'
            } );
        }
	...
	butterbean.views.register_control( 'range', {
		...
		ready: function() {
			...
			jQuery( 'input[type=range]' ).on( 'mousedown', function() {
				...
				range 			= jQuery( this );
				range_input 	= range.parent().children( '.hoocompanion-range-input' );
			...
			var hoo_range_input_number_timeout;

			function hoo_autocorrect_range_input_number( input_number, timeout ) {
				...
				clearTimeout( hoo_range_input_number_timeout );

				hoo_range_input_number_timeout = setTimeout( function() {
			...
			jQuery( 'input.hoocompanion-range-input' ).on( 'change keyup', function() {

				hoo_autocorrect_range_input_number( jQuery( this ), 1000);

			} ).on( 'focusout', function() {

				hoo_autocorrect_range_input_number( jQuery( this ), 0);

			} );

			// Handle the reset button
			jQuery( '.hoocompanion-reset-slider' ).on('click', function() {
	...
	butterbean.views.register_control( 'media', {
		...
		// Adds custom events.
		events : {
			'click .hoocompanion-add-media' 	  : 'showmodal',
		},
