 ...
 * @since  1.0.0
 */
class Hoo_Site_Options_Import {

	/**
	 * Instance of Hoo_Site_Options_Importer
	 */
	private static $_instance = null;

	/**
	 * Instanciate Hoo_Site_Options_Importer
	 */
		...
		if ( isset( $cats ) ) {
			...
			foreach ( $cats as $key => $cat ) {
				...
				if ( ! empty( $cat['slug'] ) && ! empty( $cat['thumbnail_src'] ) ) {
					...
					$image = (object) Hoo_Sites_Helper::_sideload_image( $cat['thumbnail_src'] );
	...
	private function insert_logo( $image_url = '' ) {
		...
		$data = (object) Hoo_Sites_Helper::_sideload_image( $image_url );
